...
### ✅ I-5. ECG 임베딩 과적합 — 임베딩 병목으로 해소

vf 재학습에서 val F1=0.962, test F1=0.855(격차 0.107) 관측. 원인: NSR 768차원 raw 임베딩이
소규모 P1-test 풀(130)에서 일반화 실패(비심장 클래스 rest/active/impact/hypoxia가 타격).
**해소:** raw 임베딩 *제거*(가설 `--no-embedding`) 대신 **임베딩 병목**(768→16/32→128 + 강한 dropout)으로
암기 용량을 제한 — 더 가벼운 해법. 병목 적용 학습본은 val≈test로 격차가 0.107 → ~0.01–0.04로 축소(건강).
→ 전체 제거 불요, **병목으로 종결**. 헤드라인 macro-F1(보정 후 0.94)은 병목 적용본 기준이며,
0.855는 병목 전 과적합 수치라 더 이상 유효 비교가 아니다.
